...
export default function MyIssues() {
  ...
  useEffect(() => {
    if (user) {
      // Fetch issues from JSON file and filter by user email
      fetch('/issues.json')
        .then(res => res.json())
        .then(data => {
          ...
          const sorted = userIssues.sort((a, b) => new Date(b.date) - new Date(a.date));
          setIssues(sorted);
        })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {issues.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">You haven't reported any issues yet.</p>
            <button
